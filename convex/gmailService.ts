...
// Gmail API service to replace Resend
export const sendGmailEmail = action({
  args: {
    emailType: v.string(),
    data: v.any(),
  },
  ...
  try {
    ...
    console.log('Gmail API sending email:', { from: SENDER_EMAIL, to, subject: subject.substring(0, 50) });
    
    // Create MIME message with proper formatting (based on working simple test)
    const emailMessage = [
      'MIME-Version: 1.0',
...
// Generate customer proposal email HTML (same professional design)
